...
-(void)viewWillAppear:(BOOL)animated{
    ...
    CGRect cropFrame=CGRectMake(_sourceImageView.contentFrame.origin.x,_sourceImageView.contentFrame.origin.y,_sourceImageView.contentFrame.size.width,_sourceImageView.contentFrame.size.height);
...
-(void)initCropFrame{
    _sourceImageView = [[UIImageView alloc] initWithFrame:self.view.bounds];
    _sourceImageView.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
//    _sourceImageView.autor
...
- (void)cancel:(id)sender
{
    if ([self.cropdelegate respondsToSelector:@selector(didFinishCropping:from:)]) {
        [self.cropdelegate didFinishCropping:nil from:self];
    }
}

- (void)done:(id)sender
{
    [self cropAction:sender];
}
